...
function loadDB() {
    ...
    if (file_exists($dbFile)) {
        ...
        $_SESSION['db'] = [
            'users' => [ 'admin' => ['pass' => 'admin123', 'name' => 'God', 'avatar' => '🐉', 'role' => 'admin', 'following' => [], 'inbox' => []] ],
...
function calcTitle($u) {
    ...
    foreach($_SESSION['db']['reviews'] as $aid => $arr) {
        foreach($arr as $r) {
            if($r['username'] == $u) { $revs++; if($r['score'] == 5) $glaze++; }
        }
    }
...
// Handle AJAX json body AND URL parameters
$json = file_get_contents('php://input');
$post = json_decode($json, true) ?? $_POST;
$act = $post['action'] ?? $_GET['action'] ?? '';
...
switch ($act) {
    ...
    case 'add_list':
        ...
        foreach($_SESSION['db']['collections'][$usr] as $listType => $arr) {
            foreach($arr as $k => $item) { if($item['id'] == $id) unset($_SESSION['db']['collections'][$usr][$listType][$k]); }
            $_SESSION['db']['collections'][$usr][$listType] = array_values($_SESSION['db']['collections'][$usr][$listType]);
        }
        if($t == 'top3' && count($_SESSION['db']['collections'][$usr]['top3']) >= 3) { array_shift($_SESSION['db']['collections'][$usr]['top3']); }
        $epData = ($t == 'dropped') ? $post['drop_ep'] : (($t == 'watching') ? 0 : null);
        $_SESSION['db']['collections'][$usr][$t][] = ['id' => $id, 'title' => $title, 'img' => $post['img'], 'eps' => $epData];
        if($t == 'top3') logAct("🏆 @$usr pinned $title to their Podium.");
        if($t == 'dropped') logAct("🪦 @$usr dropped $title at EP $epData.");
        saveDB(); echo json_encode(['status'=>'ok']);
        break;

    case 'update_ep':
        $usr = $_SESSION['curr_user']; $id = $post['aid']; $op = $post['op']; 
    ...
    case 'add_rev':
        ...
        $_SESSION['db']['reviews'][$id][] = [
            'id' => $_SESSION['db']['rev_id']++, 'user' => $_SESSION['db']['users'][$usr]['name'], 
            'username' => $usr, 'avatar' => $_SESSION['db']['users'][$usr]['avatar'],
            'title' => calcTitle($usr), 'txt' => $post['rev_txt'], 'score' => $post['tier_score'],
            'spoiler' => $post['is_spoiler'] ?? false, 'wtakes' => [] 
        ];
        $animeName = $post['anime_title'] ?? 'an anime';
        foreach($_SESSION['db']['users'] as $k => $uData) {
            if(in_array($usr, $uData['following'] ?? [])) { $_SESSION['db']['users'][$k]['inbox'][] = "🔔 @$usr reviewed $animeName!"; }
        }
        logAct("✍️ @$usr dropped a review for $animeName.");
        saveDB(); echo json_encode(['status'=>'ok', 'reviews'=>$_SESSION['db']['reviews'][$id]]);
        break;
    ...
    case 'w_take':
        ...
        saveDB(); echo json_encode(['status'=>'ok', 'reviews'=>$_SESSION['db']['reviews'][$aid]]);
        break;

    case 'vote_vs':
        $side = $post['side'];
        $_SESSION['db']['versus']["votes$side"]++;
        saveDB(); echo json_encode(['v1' => $_SESSION['db']['versus']['votes1'], 'v2' => $_SESSION['db']['versus']['votes2']]);
        break;
    ...
    case 'get_profile':
        ...
        if($me && $me !== $tgt) {
            ...
            foreach($tgtLater as $tShow) {
                foreach($myLater as $mShow) { if($tShow['id'] == $mShow['id']) $mutual[] = $tShow; }
            }
        }
        $followers = 0;
        ...
        echo json_encode(['status' => 'ok', 'user' => $_SESSION['db']['users'][$tgt], 'title' => calcTitle($tgt), 'followers' => $followers, 'is_following' => $is_following, 'mutual' => $mutual, 'top3' => $top3]);
        break;

    case 'toggle_follow':
        $tgt = $post['target']; $me = $_SESSION['curr_user'];
        $pos = array_search($tgt, $_SESSION['db']['users'][$me]['following']);
        if($pos !== false) { unset($_SESSION['db']['users'][$me]['following'][$pos]); } 
        else { $_SESSION['db']['users'][$me]['following'][] = $tgt; $_SESSION['db']['users'][$tgt]['inbox'][] = "👋 @$me followed you!"; }
